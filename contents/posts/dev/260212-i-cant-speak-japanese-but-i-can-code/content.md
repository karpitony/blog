...
### 가장 어려웠던 문제: 한국어 발음 제공하기

일본 가라오케에서 가장 아쉬웠던 점은
**가사 위에 한국어 발음이 없다는 것**이었습니다.

즉, 이 앱의 핵심 가치는 **일본어를 몰라도, 박자에 맞춰 부를 수 있게 해주는 것**이였습니다.

하지만 이 문제는 기존 데이터로는 해결이 불가능해보였습니다.

- 나무위키 파싱?
  → 문서 구조가 너무 자유롭고 정형화되어 있지 않음

- 기존 LRC 데이터?
  → 발음(후리가나, 루비 문자)을 포함한 경우가 거의 없음

특히 일본어는 **한자 하나가 여러 발음**을 가질 수 있기 때문에,
단순 변환으로는 정확한 발음을 얻기 어렵습니다.

이 문제를 해결하기 위해 선택한 방법이 **AI 번역**이었습니다.

### Gemini API를 활용한 발음 생성

Google의 Gemini API를 사용해
...
하지만 여기에도 아쉬운 점이 있었습니다.

AI 번역은 히라가나 / 가타가나는 잘 번역하지만, 한자는 가끔 실수를 합니다.

### 일본어의 본질적인 문제: 루비 문자

일본어에서는 한자의 발음을 명확히 하기 위해
**루비 문자(ふりがな)** 라는 방식을 사용합니다.
...
**노래 가사처럼 문학적·함축적인 문장**에서는
간혹 사람이 기대한 발음과 어긋나는 경우도 발생했습니다.
...
“그럴듯하게 동작하는 흐름”을 만들어냈다는 점에서 말입니다.
...
완벽하진 않지만,
**실제로 내가 쓰게 되는 도구를 만들었다는 점**은 분명한 성과였습니다.

언젠가 다시 이 문제를 다룬다면,

- 저작권 이슈를 정면으로 해결할 수 있는 구조로
